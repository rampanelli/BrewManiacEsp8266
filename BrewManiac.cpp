...
// PWM
  	...
  	if (now - _windowStartTime > ((unsigned long)_heatWindowSize / 10) * 255)
  	{
    	_windowStartTime += ((unsigned long)_heatWindowSize / 10) * 255;
    	//time to shift the Relay Window
  	}

  	if ((pidOutput / 254) * (((unsigned long)_heatWindowSize / 10) * 255) > now - _windowStartTime)
  	{
  		if(!_physicalHeattingOn)
  		{
  			#if SpargeHeaterSupport
